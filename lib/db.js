...
//
// Verbose: returns { hid, user, members, error } so callers can surface
// diagnostic info instead of just throwing 'No household' opaquely.
async function getHouseholdInfo() {
  const user = await getUser();
  if (!user) return { hid: null, user: null, members: null, error: null };
  ...
  if (error) return { hid: null, user, members: null, error };
  return { hid: members?.[0]?.household_id || null, user, members, error: null };
}

async function getHouseholdId() {
  const { hid } = await getHouseholdInfo();
  return hid;
}
...
export async function setBaby(form) {
  ...
  if (!info.hid) {
    // Surface enough info to diagnose. Possible causes:
    //   - user not signed in (info.user null)
    //   - RLS blocking SELECT on household_members (info.error set)
    //   - genuinely no membership row (info.members empty array)
    const detail = JSON.stringify({
      userId: info.user?.id || null,
      userEmail: info.user?.email || null,
      memberCount: info.members?.length ?? null,
      queryError: info.error?.message || null,
      queryCode: info.error?.code || null,
    });
